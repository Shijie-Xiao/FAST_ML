"""
tc_thermo.py
Copied and adapted from tropical_cyclone_risk.thermo.thermo for standalone use.
"""
...
class ThermoConfig:
    Ck: float = 1.2e-3
    Cd: float = 1.2e-3
    select_thermo: int = 1  # 1: pseudoadiabatic, 2: reversible
    select_interp: int = 2  # default to interpolation using lookup tables
    tables_dir: Path = Path(__file__).resolve().parent


CONFIG = ThermoConfig()


def set_thermo_config(**kwargs):
    """Update thermo configuration values."""
    for key, value in kwargs.items():
        if hasattr(CONFIG, key):
            setattr(CONFIG, key, value)
        else:
            raise AttributeError(f"ThermoConfig has no attribute '{key}'")
...
def sat_deficit(sst,ps,T,pm,rv):
    ...
    # Midlevel entropy
    select_thermo = CONFIG.select_thermo
...
def calc_T_rho(T,rv,rt):
    if CONFIG.select_thermo == 1:
        T_rho = T*(1+rv/pr.eps)/(1+rv)
    elif CONFIG.select_thermo == 2:
...
def CAPE_PI(sst,p_surf,p_env,T_env,r_env):
    ...
    select_interp = CONFIG.select_interp
    # If interpolation, load temperature table and coordinates
    if CONFIG.select_interp == 2:
        if select_thermo == 1:
            with np.load(CONFIG.tables_dir / 'entropy_table.npz') as entropy_table:
                p_look = entropy_table['p']
                s_look = entropy_table['s']
                T_lookup = entropy_table['T']
        elif select_thermo == 2:
            with np.load(CONFIG.tables_dir / 'entropy_table_reversible.npz') as entropy_table:
                p_look = entropy_table['p']
                s_look = entropy_table['s']
    ...
    rs = np.clip(rs, 1e-6, None)
    r_ns = np.clip(r_ns, 0.0, None)
    rh = r_ns/rs*(1+rs/pr.eps)/(1+np.clip(r_ns/pr.eps, -0.999999, None))
...
def CAPE_PI_vectorized(sst,p_surf,p_env,T_env,r_env):
    ...
    ess, rs = sat_thermo(sst,p_surf)
    # Compute RH in near-surface parcels
    # Avoid division by zero: handle zero or very small rs values
    # Use np.divide with where to suppress warnings and handle edge cases
    rs_safe = np.where(rs > 1e-10, rs, np.nan)  # Mark invalid rs as NaN
    with np.errstate(divide='ignore', invalid='ignore'):
        rh = np.divide(r_ns, rs_safe, out=np.zeros_like(r_ns), where=(rs_safe > 1e-10))
        rh = rh * (1 + rs_safe/pr.eps) / (1 + r_ns/pr.eps)
    # Set rh to 0 where rs was too small or invalid
    rh = np.where((rs > 1e-10) & np.isfinite(rh), rh, 0.0)
    # Compute reference entropies of:
    # BL parcel
    s_ns = s_unsat(T_ns,p_ns,r_ns,r_ns,select_thermo)
    # Saturated parcels at SST
    ...
    CAPE[np.isnan(CAPE)] = 0
    cape_diff = CAPEs-CAPE
    denom = np.clip(T_out_s, 1e-3, None)
    PI = np.sqrt(np.maximum(cecd * (sst / denom) * cape_diff, 0))
    PI[~np.isfinite(PI)] = 0
    return PI
